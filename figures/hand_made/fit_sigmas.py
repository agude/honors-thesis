...
for sn in sns:
    ...
    sg2[s2] = (m - M - A1*s1 - B1, merr)

writefile('sig2.dat',sg2)
...
for sn in sns:
    ...
    sg3[s3] = (m - M - A1*s1 - A2*s2 - B1 - B2, merr)

writefile('sig3.dat',sg3)
...
for sn in sns:
    ...
    sg4[s4] = (m - M - A1*s1 - A2*s2 - A3*s3 -B1-B2-B3, merr)

writefile('sig4.dat',sg4)
...
for sn in sns:
    ...
    sg5[s5] = (m - M - A1*s1 - A2*s2 - A3*s3 - A4*s4-B1-B2-B3-B4, merr)

writefile('sig5.dat',sg5)
